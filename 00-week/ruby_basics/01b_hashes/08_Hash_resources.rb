#=====  Resources  =====


  http://ruby-doc.org/core-1.9.3/Hash.html

  https://en.wikibooks.org/wiki/Ruby_Programming/Data_types
  
  THIS LESSON! - https://github.com/thefonso/ruby_lessons
  
  
#===== Excercise =====

# Download this code and play with all the examples in irb and your text editor
# let your fingers get a feel for hashes !

#===== Cheatsheets ====
# Check out the cheatsheet for Hashes
# cheatsheets/hashes.rb 

  
#=====  EXTRA FUN  =====

  Download textmate here - http://macromates.com/download

  NEW RUBY 2.0 INFO - http://globaldev.co.uk/2013/03/ruby-2-0-0-in-detail/

  pry - a very fun tool - http://pryrepl.org/
  
  #(let's play with pry if time allows)